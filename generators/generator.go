...
func (s *BaseGenerator) GetEmptySampleArray(cfg *audio.AudioConfig, n int) []float64 {
	if cfg.Stereo {
...
func (s *BaseGenerator) GetSamples(cfg *audio.AudioConfig, n int) []float64 {
	if s.GetSamplesFunc == nil {
		return s.GetEmptySampleArray(cfg, n)
	}
...
func (g *BaseGenerator) SetResult(cfg *audio.AudioConfig, result []float64, i int, v float64) {
	if !cfg.Stereo {
		result[i] = v
	} else {
		result[i*2] = v
